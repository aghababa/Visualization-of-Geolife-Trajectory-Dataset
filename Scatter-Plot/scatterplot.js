class scatterplot {
    ...
    fix() {
        ...
        let divone = d3.select('#scatter-plot-container').append('div');
        let one = divone.append('svg').classed('one', true).attr('width', figureWidth).attr('height', figureHeight);
        this.xscale = d3.scaleLinear().domain([-100, 100]).range([0, figureWidth - 100]).nice();
        this.yscale = d3.scaleLinear().domain([-100, 100]).range([0, figureHeight - 100]).nice();
        ...
        scatterplot.append('g').attr('transform', `translate(0,${figureHeight - 100})`).call(xaxis);
    ...
    drawcircles() {
        ...
        this.circles.selectAll('circle').data(this.data)
            ...
            .attr('r', d => {
                if (d.x === d['center_x_' + k] && d.y === d['center_y_' + k]) {
                    return 10
                } else {
                    return 3
                }
